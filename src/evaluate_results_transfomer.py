...
def evaluate_folder(folder, qrels):
    ...
    results.sort(key=lambda x: x[4], reverse=True)  # sort by NDCG
...
# =========================
def evaluate_all():
    result_file = RESULTS_DIR / "evaluation_transformer.txt"
    ...
    with open(result_file, "w", encoding="utf-8") as f:
        ...
        for name, qrels_path in QRELS_FILES.items():
            ...
            f.write("---- TRANSFORMER ----\n")
            trf_results = evaluate_folder(RUNS_SEARCH_TRANSFORMER_DIR, qrels)

            for file, p, r, f1_score, ndcg in trf_results:
                f.write(
                    f"{file}\n"
            ...
            # best by NDCG
            if trf_results:
                best = max(trf_results, key=lambda x: x[4])
